...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features(self, x: Tensor | List[Tensor], mask: Optional[Tensor] = None) -> List[Dict[str, Tensor]]:
        if isinstance(x, torch.Tensor):
            return self.forward_features_list([x], [mask])[0]
        else:
    ...
    def forward(self, *args, is_training: bool = False, **kwargs) -> List[Dict[str, Tensor]] | Tensor:
        ...
        if self.return_all_tokens:
            return torch.cat([ret["x_norm_clstoken"].unsqueeze(1), ret["x_norm_patchtokens"]], dim=1)
        if is_training:
            return ret
        else:
