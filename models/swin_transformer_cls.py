...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, cls_win, mask=None):
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))  # (B_, nH, N_all, N_all)

        # relative bias solo para tokens de ventana (no para el CLS)
        ...
        attn[..., :N, :N] = attn[..., :N, :N] + rel_bias.unsqueeze(0)

        if mask is not None:
            nW = mask.shape[0]
            # extendemos a (nW, N+1, N+1) con ceros en la última fila/col
            ext = x.new_zeros(nW, N_all, N_all)
            ext[:, :N, :N] = mask
            attn = attn.view(B_ // nW, nW, self.num_heads, N_all, N_all)
            attn = attn + ext.unsqueeze(0).unsqueeze(2)
            attn = attn.view(-1, self.num_heads, N_all, N_all)
            
        attn = self.softmax(attn)
        attn_out = attn

        x = (attn @ v).transpose(1, 2).reshape(B_, N_all, C)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # proyector de CLS al dim del siguiente stage (si hay downsample)
        self.cls_down = nn.Linear(dim, dim*2, bias=True) if downsample is not None else None
    ...
    def forward_with_features(self, x, stage_cls=None):
        ...
        if self.downsample is not None:
            x = self.downsample(x)
            if stage_cls is not None:
                stage_cls = self.cls_down(stage_cls)
        return x, fea, stage_cls
...
class SwinTransformerCLS(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=0,
        ...
        self.stage_cls_tokens = nn.ParameterList([
            nn.Parameter(torch.zeros(1, 1, int(embed_dim * 2 ** i)))
